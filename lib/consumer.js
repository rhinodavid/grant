...
module.exports = ({ config }) => {
      ...
      if (provider.oauth === 1) {
        oauth1
          ...
          .then(result => {
            ...
            oauth1
              .authorize(provider, result.body)
  ...
  var callback = ({ session, query, state = {} }) =>
    new Promise(resolve => {
      var provider = _config.provider(config, session, state);
      var response = transport(provider, session, state);
      ...
      if (provider.oauth === 1) {
        oauth1
          .access(provider, session.request, query)
          .then(data => resolve(response(data)))
          .catch(err => resolve(response(err)));
      } else if (provider.oauth === 2) {
        oauth2
          .access(provider, query, session)
          .then(data => resolve(response(data)))
          .catch(err => resolve(response(err)));
      } else {
